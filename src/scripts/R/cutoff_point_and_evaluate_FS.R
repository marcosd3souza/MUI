    ...
    ### TIME EXECUTION : ~  7.883921 mins ### METHOD: SPEC; BASE: GDS183; CONFIGS_NON_ZERO: 42

    methods = list("SPEC");#, "iDetect", "SPEC", "GLSPFS", "JELSR", "SparseKMeans");
    bases = list("GDS183");#,"GDS1210", "GDS2545", "GSE68936", "GSE82009");
            ...
            for(file in scores_files)
            {
                ...
                if(sum(data) != 0)
                {
                    ...
                    for(index in 1:len_features)
                    {
                        ...
                        stats <- cluster.stats(dgene, km$cluster, alt.clustering = as.integer(classes$disease_state), G2 = TRUE, G3 = TRUE);                    
                        ...
                        tb <- as.data.frame(confusion_matrix$byClass);
